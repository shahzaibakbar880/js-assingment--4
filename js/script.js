...
let cities = ["Islamabad", "Lahore", "Sialkot", "Multan", "Bahawalpur" ]
...
const checkCity = () => {
    ...
    if (!checkCity) {
        showNotification("please enter your city name 🙏 in input", "error")
        return;
    }
...
const findWord = () => {
    let word = document.getElementById("inputField").value;
    if (!word) {
        showNotification("please enter your  word 🙏 ", "error")
        return;
    }
    ...
    if (findWord !== -1) {
        ...
        document.getElementById("output").innerHTML = `  sorry 🥺 the word you entered is not found`
    }
}
...
const replaceWord = () => {
    ...
    if (!newWord) {
        showNotification("please enter a new word ", "error")
        return;
    }
